...
namespace WebStore.Areas.Administrator.Controllers
{
    public class SaleController : Controller
    {
        ...
        [HttpGet]
        public async Task<ActionResult> GetCustomerOrdersDetails(Guid Id)
        {
            var orders = await salesService.GetSalesOrdersDetailsByOrderId(Id);
        ...
        public async Task<ActionResult> SaveTransaction(SalesDTO salesDTO)
        {
            ...
            if (result != null)
            {
                ...
                return Json(new { success = true, responseText = salesDTO.OrderNumber }, JsonRequestBehavior.AllowGet);
        ...
        public async Task<ActionResult> GetSales(Models.JqueryDatatableParam param)
        {
            ...
            {
                customers = customers.Where(x => x.ProductName.ToLower().Contains(param.sSearch.ToLower())
            ...
            {
                customers = sortDirection == "asc" ? customers.OrderBy(c => c.ProductName).ToList() : customers.OrderByDescending(c => c.ProductName).ToList();
            }
            ...
            {
                Func<SalesDetailsDTO, string> orderingFunction = e => sortColumnIndex == 0 ? e.ProductName : sortColumnIndex == 1 ? e.OrderNumber : e.SellingPrice.ToString();
        ...
        public async Task<ActionResult> GetAllCustomerOrders(Models.JqueryDatatableParam param)
        {
